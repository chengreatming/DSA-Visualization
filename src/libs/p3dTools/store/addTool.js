...
const addToolForElement = function (element, ApiTool, props) {
  ...
  // To a new element).
  if (typeof ApiTool !== 'function') return false
  const tool = new ApiTool(props);
  const toolAlreadyAddedToElement = getToolForElement(element, tool.name);

  if (toolAlreadyAddedToElement) {
    console.warn('%s has already been added to the target element', tool.name);
...
const addTool = function (ApiTool, props) {
  ...
  store.state.enabledElements.forEach(obj => {
    addToolForElement(obj.domElement, ApiTool, props);
  });
};
...
const _addToolGlobally = function (ApiTool, props) {
  ...
  if (toolAlreadyAddedGlobally) {
    console.warn('%s has already been added globally', tool.name);
