...
import { getToolState, removeToolState } from '../stateManagement'
...
const setToolActive = function (toolName, options, interactionTypes) {
  ...
  store.state.enabledElements.forEach(obj => {
    setToolActiveForElement(obj.domElement, toolName, options, interactionTypes);
  });
};
...
function setToolModeForElement(mode, changeEvent, element, toolName, options) {
  ...
  if (!tool) {
    console.warn('Unable to find tool "%s" for enabledElement', toolName);
  ...
  // Remove unCompleted measurement
  if (mode === 'passive') {
    const toolState = getToolState(element, toolName)
    toolState && toolState.data && toolState.data.forEach(toolData => {
      if (toolData.isCompleted === false) {
        removeToolState(element, toolName, toolData)
      }
    })
  }
...
function setToolMode(mode, changeEvent, toolName, options) {
  ...
  store.state.enabledElements.forEach(obj => {
    setToolModeForElement(mode, changeEvent, obj.domElement, toolName, options);
  });
}
...
function _resolveInputConflicts(element, tool, options, interactionTypes) {
  ...
  activeToolsForElement.forEach(t => {
    ...
    if (!toolHasAnyActiveInteractionType) {
      // console.log('Setting tool %s\'s to PASSIVE', t.name);
