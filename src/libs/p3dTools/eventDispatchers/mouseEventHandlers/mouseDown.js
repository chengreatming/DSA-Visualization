...
export default function (evt) {
  ...
  const coords = evt.detail.currentPoints.screen;
  ...
  activeTools = activeTools.concat(activeAndPassiveTools.filter(
    tool => tool.specialSupportedCallback.includes('mouseDown')
  ))
  ...
  if (eventData.buttons != 1) return false
  ...
  if (annotationToolsWithMoveableHandles.length > 0) {
    ...
    if (!handle.unmoveable && typeof firstToolWithMoveableHandles.handleSelectedCallback === 'function') {
    ...
    tool => {
      ...
      const isNearPoint =
        ...
        toolState.data.some(data =>
          tool.pointNearTool(enabledElement, data, coords, 'mouse')
        );
      return isNearPoint;
    }
  );
  ...
  if (annotationToolsWithPointNearClick.length > 0) {
    ...
    const firstAnnotationNearPoint = toolState.data.find(data =>
      firstToolNearPoint.pointNearTool(enabledElement, data, coords)
    );
    if (typeof firstToolNearPoint.toolSelectedCallback === 'function') {
